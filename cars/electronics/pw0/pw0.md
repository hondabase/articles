...
## Overview
As one of Honda's earliest VTEC ECUs, the PW0 features a robust internal layout that includes dual knock sensor inputs (JDM versions) and uses an OKI-based microcontroller architecture. Like the similar [PR3 ECU](/cars/electronics/pr3), the PW0 is highly popular among enthusiasts restoring or tuning 4th-generation Civics and CRXs.
...
The final calculated injector pulse-width value uses this multiplier to scale the raw fuel map cell value into the final injection duration commanded by the ECU. Refer to the [understanding maps guide](/cars/electronics/understanding-maps) for a step-by-step example of load scaling.
