...
Re: P30 Excel Map Calculator Author: Joseph Davis Date: 11-12-02 22:00 Sammy, all it is doing is displaying the hex or binary values in decimal. Open a [ROM](/cars/electronics/rom) that you have table locations for in the program. Offset is the memory location you're looking for, that the table begins at. Colonnes is how many columns - kinda critical for newbie map recognition. Maps are 15X17 for [OBD](/cars/electronics/obd)0 and 10X20 for [OBD](/cars/electronics/obd)1 - that means Columns X Rows. Set to 15 if [OBD](/cars/electronics/obd)0 and 10 if [OBD](/cars/electronics/obd)1. Step is how many locations the program jumps when you press the up/down arrows on offset (great for searching for a table in a new [ROM](/cars/electronics/rom)). Precision is just a multiplier. If you set it to 1 you'll get the real values from 0-255 in decimal instead of 00-FF in hex - since we learn decimal in school it is easier to work with at first. If you sit down with a PM6.bin and the CRX map calculator.xls you'll have zero problems identifying the maps, and the map calculator will have the same decimal as what you see in the Analyser.exe. The P30 kids came up with a similar Excel spreadsheet if you'd rather start there. You see how the values distibute themselves logically across the ignition map? The fuel is doing the same, but the further across the columns you go you have to start using that column's multiplier at the bottom of the map or else it looks like the map starts lean, goes rich, then starts lean + goes rich a couple times across it. | **Attachment:** | **Modify:** | **Size:** | **Date:** | **Who:** | **Comment:** | | :--- | :--- | :--- | :--- | :--- | :--- | | ![](/pgmfi/wiki/assets/icn/exe.gif) [Analyze.exe](Analyze.exe) | mod | 589312 | 22 Jul 2004 - 16:10 | blundar | Nico's analyze.exe |
