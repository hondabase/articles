...
4. **Visual Compliance:** Ensure all engine swap components comply with local guidelines. For BAR inspections, all aftermarket parts (headers, intake manifolds) must display a valid CARB Executive Order (EO) number, and there should be no visible tuning hardware or datalogging cables exposed. Refer to the [OBD generations guide](/cars/electronics/obd) for factory configuration details.
