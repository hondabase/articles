...
- **Distributor Position Sensors:** Ensure the ECU is configured to match the distributor sensor signals (CYP, CKP, TDC) coming from the engine block. Refer to the [cylinder position sensor guide](/cars/electronics/cylinder-position-sensor) for detailed sensor diagnostics.
